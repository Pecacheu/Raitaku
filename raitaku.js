...
async function getFaGallery(user, page = 1) {
	...
	const fa = {url: `${FA_URI}/gallery/${user}/${page}`, user, page, posts: []};
	try {
		//Fetch page
	...
	} catch(e) {throw errAt(`Parse FA Gal @${user} p${page}`, e)}
	return fa;
}

async function getFaPost(id, noDesc) {
	print(C.magenta('Loading FA #' + id));
	const fa = {url: `${FA_URI}/view/${id}`};
	try {
		//Fetch post
		let dom = await httpReq(fa.url, 0, FA_AUTH);
		dom = parse(dom);
		//Check system err
		let el = dom.querySelector('.notice-message');
		if(el) {
			el = el.textContent.trim().match(/^(.+?)\n(.+)$/);
			throw `${el[1].trim()}: ${el[2].replace(/\n/g, ' ').trim()}`;
		}
		//File info
		el = dom.querySelector('#submission-options');
		if(!el) throw 'Options';
		el = el.children.find(e => e.textContent === 'Download');
		if(!el) throw 'Download button';
		el = el.attributes.href;
		fa.file = faAbsURL(el);
		el = el.slice(el.lastIndexOf('/') + 1);
		const fn = R_FN.exec(el);
		fa.fn = fn ? fn[1] : el;
		//Post info
		...
		el = dom.querySelector('.submission-description-text');
		if(!el) throw 'Desc';
		...
		//Content stats
		el = dom.querySelector('.submission-content-stats')?.children;
		if(el?.length !== 2) throw 'Content Stats';
		const c1 = el[0].children, c2 = el[1].children, dat = {};
		if(c1.length !== c2.length) throw 'Stat Data';
		c1.forEach((n, i) => dat[toKeyFmt(n.textContent)] = c2[i].textContent.trim());
		if(!(fa.cat = dat.category)) throw 'Category';
		if(!(fa.type = dat.theme)) throw 'Theme';
		if(!(fa.species = dat.species)) throw 'Species';
		//Page stats
		el = dom.querySelector('.submission-page-stats')?.children;
		if(!el) throw 'Page Stats';
		for(const e of el) fa[toKeyFmt(e.children[1].textContent)] = e.children[0].textContent.trim();
		if(!fa.rating) throw 'Rating';
		//Tags
		el = dom.querySelector('.submission-tags');
		if(!el) throw 'Tags';
		fa.tags = {};
		el.querySelectorAll('.tags:not(:has(.tag-invalid))')
			.forEach(e => fa.tags[toKeyFmt(e.textContent)] = 1);
		//Folders
		el = dom.querySelector('.folder-list-container');
		if(!el) throw 'Folders';
		el = el.querySelectorAll('.submission-folder');
		fa.folders = [];
		if(el) for(let e of el) if(e.tagName === 'DIV') {
			e = e.querySelector('span');
			if(!e) throw 'Folder Name';
			fa.folders.push(e.textContent.trim());
		}
	} catch(e) {throw errAt(`Parse FA #${id}`, e)}
	return fa;
}

async function getFaUser(user) {
	print(C.dim(`Checking FA User @${user}`));
	const fa = {url: `${FA_URI}/user/${user}`, user};
	try {
		//Fetch page
		let dom = await httpReq(fa.url, 0, FA_AUTH);
		dom = parse(dom);
	...
	} catch(e) {throw errAt(`Parse FA @${user}`, e)}
	return fa;
}
	...
	for(; i < l; ++i) {
		...
		case 'A':
			...
			if(e.classList.contains('iconusername')) { //User
				const id = R_ID.exec(n)[1];
				t = n;
				if(UserCache[id] == null) { //FA -> Itaku User
					const cl = {[id]: 1};
					let fc, u;
					try {fc = await getFaUser(id)} catch(e) {
...
async function newItakuPost(dat) {
	...
	r = await httpReq(IT_API + '/posts/', {title: dat.title,
		content: dat.desc || '',
		tags,
		maturity_rating: r,
		gallery_images: dat.ids,
		visibility: 'PUBLIC'}, IT_AUTH, 'POST');
...
async function addTag(tag, tags, te) {
	...
	const tm = tag.replace(R_TM, '');
	t = t.results.find(r => r.name.replace(R_TM, '') === tm);
...
function fixSuffix(tag, name) {
	...
	const nl = name.length, s = tag[tag.length - nl - 1];
	if(!s || s === '_') return tag;
...
async function promptWarn(w) {
	warn(w);
	if(SkipWarn) return;
	const a = (await prompt('Continue? (Y/N) ')).toLowerCase();
...
function toFormData(obj) {
	...
	for(const k in obj) fd.set(k, typeof obj[k] === 'string'
		? obj[k]
...
async function httpReq(uri, data, hdr, meth = 'GET', blob) {
	...
	if(r.status !== 200 && r.status !== 201) {
		const b = await r.text(), q = uri.indexOf('?');
...
async function transferOne(id, doSets) {
	...
	if(doSets && fa.set) { //Set / Comic
		...
		//Upload all
		const ids = [], tags = {}, fp = fa;
		let d, t;
		while(true) {
			d = PostedIDs[id];
			if(d) { //Already posted
				if(ids.indexOf(d.id) !== -1) {
					warn('Recursive loop; Breaking cycle');
					break;
				}
			} else { //New post
		...
		//Create post set
		return newItakuPost({title: fp.title,
			desc: fp.desc,
			rating: fp.rating,
			tags,
			ids});
...
async function transfer(idSt, idEnd, doSets) {
	...
	const d = await getFaPost(idSt, true), gal = [];
	let pg = 1, i, s, e, sp, ep;
	for(; !sp || !ep; ++pg) {
